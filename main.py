...
def main(
    ...
    if from_cmd:
        # this check is needed only when main function is called from `if __name__ == '__main__'`
        # because it will pass `None` as parameter
    ...
    ffmpeg_arguments_obj, naming_1 = read_presets(presets)
    if naming is None:
        naming = naming_1
    ...
    while True:
        ...
        try:
            ...
            print_msg(ex.__str__())  # I feel like I need a try-catch here, but I don't know why. Maybe I'm wrong.
        if debug:
        ...
        while len(files_in) > 0:
            ...
            try:
                ...
                files_in.clear()
                continue
            source_duration, source_frame_count = get_episode_duration(file_in, ffprobe_path)
            if debug:
                print(f"input duration: {source_duration}; input frames: {source_frame_count}")
            ...
            if no_verify_source:
                ...
                print_msg(
                    f"Unable to verify source. Cannot find CRC checksum."
                )
            ...
            if ffmpeg_arguments_obj.hardware_acceleration is not None:
                ...
                command.insert(1, ffmpeg_arguments_obj.hardware_acceleration)
            ...
            encoded_duration, _ = get_episode_duration(output_file, ffprobe_exec=ffprobe_path)
...
args.add_argument('--presets', dest='presets', nargs=1, type=str,
                  required=False, help='Encode presets for encoding, in JSON format')
...
args.add_argument('--schedule-encode-hour', dest="schedule_encode_hour", nargs='+', type=int,
                  required=False, help='TO BE DONE')
...
if __name__ == "__main__":
    ...
    presets = None if args1.presets is None else Path(args1.presets[0])
